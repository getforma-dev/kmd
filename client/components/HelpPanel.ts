  ...
  const backdrop = h('div', {
    ...
    h('div', {
      ...
      Section('CLI Commands',
        CmdRow('kmd', 'Start the dev server (opens browser)'),
        CmdRow('kmd --port 8080', 'Use a custom port'),
        CmdRow('kmd --no-open', 'Start without opening browser'),
        CmdRow('kmd --name "MyProject"', 'Set workspace name'),
        CmdRow('kmd add <path>', 'Add a project root to workspace'),
        CmdRow('kmd add ../api ../web', 'Add multiple roots at once'),
        CmdRow('kmd remove <path>', 'Remove a root from workspace'),
        CmdRow('kmd list', 'Show workspace info and roots'),
        CmdRow('kmd init', 'Create .kmd/ without starting server'),
      ...
      Section('Tips',
        h('div', { style: 'font-size: 12px; color: var(--gruvbox-gray); line-height: 1.7;' },
          ...
          h('p', { style: 'margin: 0 0 6px;' },
            h('strong', { style: 'color: var(--gruvbox-fg);' }, 'Config: '),
            'Workspace settings are in ',
            h('code', { style: 'font-size: 11px; color: var(--accent, var(--gruvbox-yellow));' }, '.kmd/workspace.json'),
            '. Hidden ports are in ',
            h('code', { style: 'font-size: 11px; color: var(--accent, var(--gruvbox-yellow));' }, '.kmd/ports.json'),
            '.',
          ),
          h('p', { style: 'margin: 0 0 6px;' },
            h('strong', { style: 'color: var(--gruvbox-fg);' }, 'Port auto-increment: '),
            'If port 4444 is busy, kmd tries 4445, 4446, etc.',
          ),
          h('p', { style: 'margin: 0;' },
