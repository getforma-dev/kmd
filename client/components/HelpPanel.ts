  ...
  const backdrop = h('div', {
    ...
    h('div', {
        ...
        h('div', { style: 'font-size: 13px; color: var(--gruvbox-fg); line-height: 1.8; margin-bottom: 4px;' },
          h('div', { style: 'display: flex; gap: 10px; align-items: flex-start; margin-bottom: 10px; padding: 8px 10px; background: var(--gruvbox-bg-hard); border-radius: 4px;' },
            ...
            h('div', null,
              ...
              h('div', { style: 'font-size: 12px; color: var(--gruvbox-gray);' },
                ...
                ' in any directory. Scans docs, scripts, ports. Close it and nothing is left behind.',
              ),
            ),
          ),
          h('div', { style: 'display: flex; gap: 10px; align-items: flex-start; padding: 8px 10px; background: var(--gruvbox-bg-hard); border-radius: 4px;' },
            ...
            h('div', null,
              ...
              h('div', { style: 'font-size: 12px; color: var(--gruvbox-gray);' },
                'Run ',
                h('code', { style: 'color: var(--accent, var(--gruvbox-yellow)); font-size: 11px;' }, 'kmd init'),
                ' to create a workspace. Fixed port, multiple project roots, hot-reload. Add projects with ',
                h('code', { style: 'color: var(--accent, var(--gruvbox-yellow)); font-size: 11px;' }, 'kmd add ../project'),
                '.',
              ),
            ),
          ),
      ...
      Section('CLI Commands',
        ...
        CmdRow('kmd add <path>', 'Add project root to workspace'),
        CmdRow('kmd remove <path>', 'Remove project root from workspace'),
      ...
      Section('Tips',
        h('div', { style: 'font-size: 12px; color: var(--gruvbox-gray); line-height: 1.7;' },
          h('p', { style: 'margin: 0 0 6px;' },
            ...
            'Workspaces use port 4444 (fixed). Ephemeral sessions auto-pick from 4445-4460. Use ',
            h('code', { style: 'font-size: 11px; color: var(--accent, var(--gruvbox-yellow));' }, '--port'),
            ' to override.',
          ),
          h('p', { style: 'margin: 0 0 6px;' },
            h('strong', { style: 'color: var(--gruvbox-fg);' }, 'Workspace settings: '),
            'Click the gear icon in the sidebar, or type "workspace" in the command palette (',
            h('kbd', { class: 'kbd', style: 'font-size: 10px;' }, `${mod}K`),
            ').',
          ),
          h('p', { style: 'margin: 0 0 6px;' },
            h('strong', { style: 'color: var(--gruvbox-fg);' }, 'Check status: '),
            'Run ',
            h('code', { style: 'font-size: 11px; color: var(--accent, var(--gruvbox-yellow));' }, 'kmd status'),
            ' to see if a workspace server is running.',
          ),
          h('p', { style: 'margin: 0;' },
